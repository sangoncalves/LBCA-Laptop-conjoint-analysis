...
The preference shares are in logit scale it difficult to interpret it. Thus we use sensitivity analysis by stuying how preference share changes as each level of each attributes are changed one after the other. This function intakes the model, attributes, reference profile: one which is used to study the sensitivity of attributes and competitive set of profiles as the input. 
