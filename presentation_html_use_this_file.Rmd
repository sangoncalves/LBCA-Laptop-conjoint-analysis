...
As can be noticed, there was hardly any changes. The attribute with major changes kept being the memory (1T)

```{r Sensitivity Function2, include=FALSE}
sensitivity.mnl.mixed <- function(model, attrib, base.data, competitor.data) {
  # Function for creating data for a preference share-sensitivity chart
  # model: mlogit object returned by mlogit() function
  # attrib: list of vectors with attribute levels to be used in sensitivity
  # base.data: data frame containing baseline design of target product
...
```{r Trade-off attributes Mixed model graph - MNL, echo=FALSE}
options(scipen = 999)
tradeoff.mixed <- sensitivity.mnl.mixed(lm2.mixed2, attributes, base.data, competitor.data)
print(tradeoff.mixed)
barplot(tradeoff.mixed$increase, horiz=FALSE, names.arg=tradeoff.mixed$level,
        ylab="Change in Share for the Planned Product Design", 
        ylim=c(-0.1,0.4))
grid(nx=NA, ny=NULL)
```

The graph above is the sensitivity chart with price - 1k, RAM - 8GB, memory - 256GB, processor -	i5, weight - 1kg, screensize -	13" as reference profile configuration. We can see that, increasing the memory from 256GB to 1TB would also increase the preference shares by 14% and increasing the screensize to 16" can also increase the preference share by 3%. Any change in RAM size, processor or weight will have a negative effect on the preference shares. 
...
In summary, the finding of our analysis is that the attribute that represents the biggest change in preference was memory (1T) and the costumers are homogeneous regarding this preference. The attributes that presented features of heterogeneity are weight (1.5) and processor (i5). The model chosen as the best representing our data was "Random effects + all correlated" (lm2.mixed2). Finally, is necessary to analyse if would be possible to increase the memory of our desired product without compromising much the price in order to increase the preference share.
