...
```
## Correlated model
It is reasonable to think that the variables can be correlated. In order to verify that, we are going to create a model that takes in consideration random effects (variation according to respondents) and that the random parameters are correlated.
First we consider correlation among all pair of variables and analyse its covariance.
...
By analysing the signs from variance-covariance, we can update the model to contain just the variables that are correlated.
